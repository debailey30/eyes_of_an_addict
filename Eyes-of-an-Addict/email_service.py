...
def send_newsletter_campaign(subject, content, recipient_emails=None):
    """
    ...
    try:
        ...
        # If no specific recipients provided, send to all active subscribers
        if recipient_emails is None:
            subscribers = EmailSubscriber.query.filter_by(is_active=True).all()
            recipient_emails = [(sub.email, sub.name) for sub in subscribers]
        ...
        for email, name in recipient_emails:
            try:
                ...
                personalized_content = content.replace("{{greeting}}", greeting)
                ...
                sg.send(message)
                sent_count += 1
                logging.info(f"Newsletter sent to: {email}")
                
            except Exception as e:
                failed_count += 1
                logging.error(f"Failed to send newsletter to {email}: {e}")
        
        return True, f"Newsletter sent to {sent_count} subscribers. {failed_count} failed."
        
    except Exception as e:
        logging.error(f"Newsletter campaign error: {e}")
        return False, f"Campaign failed: {str(e)}"
