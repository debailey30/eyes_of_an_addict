...
class NewsletterForm(FlaskForm):
    """Form for sending newsletter campaigns"""
    ...
    content = TextAreaField('Newsletter Content (HTML)', validators=[
        DataRequired(message="Please enter newsletter content")
    ], render_kw={
        "rows": 15, 
        "placeholder": "Use {{greeting}} to personalize with subscriber names"
    })
    
    preview_email = EmailField('Preview Email (Optional)', validators=[
        Email(message="Please enter a valid email address")
    ])
    
    send_preview = SubmitField('Send Preview')
    send_campaign = SubmitField('Send to All Subscribers')
